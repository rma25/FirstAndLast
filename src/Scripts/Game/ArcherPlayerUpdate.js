function ArcherController(parent) {
    ...
    var archerPlayer = game.playerMap[CurrentClientId];
    var playerInfo = {};
    ...
    if (!IsGameOver && archerPlayer != null && archerPlayer != undefined) {
        var currentArcher = (game.DoesPlayerHasStrengthBuff ? 'archer2-' : 'archer1-');
        ...
        if (Cursors.left.isDown) {
            ...
            archerPlayer.setVelocityX((game.DoesPlayerHasSpeedBuff ? -350 : -240) * (parent.run.isDown ? 1.55 : 1));
        ...
        //Player Not Moving
            ...
            if (parent.mainAttack1.isDown && !game.IsArrowShot) {
                ...
                if (game.IsMainPlayerFacingLeft) {
                    ...
                    if (archerPlayer.anims.currentFrame.index >= 10) {
                        ...
                        game.ArrowsLeft.enableBody(true, archerPlayer.x - 16, archerPlayer.y, true, true);
                        ...
                        game.DidArcherAttackOnce = archerPlayer.anims.currentFrame.textureKey.includes('attack1');
                    }
                }
                else {
                    ...
                    if (archerPlayer.anims.currentFrame.index >= 10) {
                        ...
                        game.ArrowsRight.enableBody(true, archerPlayer.x + 16, archerPlayer.y, true, true);
            ...
            //Trap
            else if (parent.specialAttack1.isDown && !game.IsArcherSpecialAttack1Used) {
                game.IsPlayerIdle = false;
                archerPlayer.anims.play(currentArcher + 'specialAttack1-' + (game.IsMainPlayerFacingLeft ? 'left' : 'right'), true);

                //Only attack on the last animation frame
                if (archerPlayer.anims.currentFrame.isLast) {
                    game.ArcherSpecialAttack1 = CreateArcherSpecialAttack1(parent, archerPlayer);
                    game.ArcherSpecialAttack1.play('archer-specialAttack-1');

                    if (game.IsMainPlayerFacingLeft) {
                        game.ArcherSpecialAttack1.enableBody(true, archerPlayer.x - (game.ArcherSpecialAttack1.displayWidth / 2), archerPlayer.y + (archerPlayer.displayHeight / 2) - 3, true, true);
                    }
                    else {
                        game.ArcherSpecialAttack1.enableBody(true, archerPlayer.x + (game.ArcherSpecialAttack1.displayWidth / 2), archerPlayer.y + (archerPlayer.displayHeight / 2) - 3, true, true);
                    }
            ...
            //Cannon Ball
            else if (parent.specialAttack2.isDown && !game.IsArcherSpecialAttack2Used) {
                game.IsPlayerIdle = false;
                archerPlayer.anims.play(currentArcher + 'specialAttack1-' + (game.IsMainPlayerFacingLeft ? 'left' : 'right'), true);

                //Only attack on the last animation frame
                if (archerPlayer.anims.currentFrame.isLast) {
                    game.ArcherSpecialAttack2_Cannon = CreateArcherSpecialAttack2_Cannon(parent, archerPlayer);
                    game.ArcherSpecialAttack2_Cannon.play('archer-specialAttack-2-0-' + (game.IsMainPlayerFacingLeft ? 'left' : 'right'));
                    game.IsCannonFacingLeft = game.IsMainPlayerFacingLeft;

                    if (game.IsMainPlayerFacingLeft) {
                        game.ArcherSpecialAttack2_Cannon.enableBody(true, archerPlayer.x - (game.ArcherSpecialAttack2_Cannon.displayWidth / 2), archerPlayer.y + 15, true, true);
                    }
                    else {
                        game.ArcherSpecialAttack2_Cannon.enableBody(true, archerPlayer.x + (game.ArcherSpecialAttack2_Cannon.displayWidth / 2), archerPlayer.y + 15, true, true);
                    }
            ...
            else if (game.IsMainPlayerFacingLeft && archerPlayer.body.touching.down && !archerPlayer.body.isMoving) {
                archerPlayer.anims.play(currentArcher + 'idle' + (game.IsPlayerIdle ? '2' : '1') + '-left', true);
            }
            else if (!game.IsMainPlayerFacingLeft && archerPlayer.body.touching.down && !archerPlayer.body.isMoving) {
                archerPlayer.anims.play(currentArcher + 'idle' + (game.IsPlayerIdle ? '2' : '1') + '-right', true);
            }
        }

        //Player Jump
        if ((Cursors.up.isDown || parent.jumpAlt.isDown) && archerPlayer.body.touching.down) {
            archerPlayer.setVelocityY((game.DoesPlayerHasStrengthBuff ? -670 : -550));
            archerPlayer.anims.play(currentArcher + 'jump-' + (game.IsMainPlayerFacingLeft ? 'left' : 'right'), true);
...
function ShootCannonBall(parent, archerPlayer) {
    if (game.ArcherSpecialAttack2_Cannon != null && game.ArcherSpecialAttack2_Cannon != undefined && game.ArcherSpecialAttack2_Cannon.anims != null && game.ArcherSpecialAttack2_Cannon.anims != undefined) {
        if (game.ArcherSpecialAttack2_Cannon.anims.currentFrame.isLast) {
            game.ArcherSpecialAttack2_Ball = CreateArcherSpecialAttack2_Ball(parent, archerPlayer);
            game.ArcherSpecialAttack2_Ball.play('archer-specialAttack-2-1');

            if (game.IsCannonFacingLeft) {
                game.ArcherSpecialAttack2_Ball.enableBody(true, game.ArcherSpecialAttack2_Cannon.x - (game.ArcherSpecialAttack2_Ball.displayWidth / 2), game.ArcherSpecialAttack2_Cannon.y, true, true);
                game.ArcherSpecialAttack2_Ball.setVelocityX(game.DoesPlayerHasStrengthBuff ? -700 : -600);
                game.ArcherSpecialAttack2_Ball.setAccelerationX(game.DoesPlayerHasStrengthBuff ? -300 : -150);
            }
            else {
                game.ArcherSpecialAttack2_Ball.enableBody(true, game.ArcherSpecialAttack2_Cannon.x + (game.ArcherSpecialAttack2_Ball.displayWidth / 2), game.ArcherSpecialAttack2_Cannon.y, true, true);
                game.ArcherSpecialAttack2_Ball.setVelocityX(game.DoesPlayerHasStrengthBuff ? 700 : 600);
...
function UpdateArcherAttacks() {
    ...
    if (game.ArcherSpecialAttack2_Ball != null && game.ArcherSpecialAttack2_Ball != undefined) {
        if (game.IsMainPlayerFacingLeft) {
...
function DestroyArcherSpecialAttack1() {
    if (game.ArcherSpecialAttack1 != null && game.ArcherSpecialAttack1 != undefined && game.ArcherSpecialAttack1.body != null && game.ArcherSpecialAttack1.body != undefined) {
...
function DestroyArcherAttacks() {
    //In case it hits a wall (side of the window)        
    if (game.ArcherSpecialAttack2_Ball != null && game.ArcherSpecialAttack2_Ball != undefined && game.ArcherSpecialAttack2_Ball.body != null && game.ArcherSpecialAttack2_Ball.body != undefined) {
        if (game.ArcherSpecialAttack2_Ball.body.onWall() && !game.ArcherSpecialAttack2_Ball.body.onFloor() && game.ArcherSpecialAttack2_Ball.body.enable) {
