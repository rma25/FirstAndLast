...
function CreateNewPlayer(playerId, playerX, playerY) {
  game.jumpVelocityNormal = -800;
  game.jumpVelocityHigh = -750;
...
function LiveBackground(parent) {
  ...
  parent.add
    .sprite(CenterWidth / 3, WindowHeight - algHeight, "alg0")
    .play("liveAlg");
  parent.add
    .sprite(CenterWidth, WindowHeight - algHeight, "alg0")
    .play("liveAlg");
  parent.add
    .sprite(CenterWidth + algWidth * 5, WindowHeight - algHeight, "alg0")
    .play("liveAlg");
  parent.add
    .sprite(CenterWidth + algWidth * 13, WindowHeight - algHeight, "alg0")
    .play("liveAlg");
}
...
function Platform(parent) {
  ...
  Platforms.create(
    CenterWidth + GroundWidth,
    CenterHeight + GroundHeight,
    "ground"
  );
  ...
  //Left
  Platforms.create(
    CenterWidth - GrassGroundWidth,
    firstHeight,
    "floatingGround1"
  );
  Platforms.create(
    CenterWidth - GrassGroundWidth * 2,
    firstHeight,
    "floatingGround1"
  );
  ...
  Platforms.create(
    CenterWidth + GrassGroundWidth * 3,
    firstHeight,
    "floatingGround1"
  );
  Platforms.create(
    CenterWidth + GrassGroundWidth * 4,
    firstHeight,
    "floatingGround1"
  );
  /***************/
  ...
  //Right
  Platforms.create(
    CenterWidth + GrassGroundWidth * 7,
    secondHeight + GrassGroundHeight * 1.5,
    "floatingGround1"
  );
  Platforms.create(
    CenterWidth + GrassGroundWidth * 6,
    secondHeight + GrassGroundHeight * 1.5,
    "floatingGround1"
  );
  Platforms.create(
    CenterWidth + GrassGroundWidth * 5,
    secondHeight + GrassGroundHeight * 1.5,
    "floatingGround1"
  );
  /***************/
  ...
  Platforms.create(
    CenterWidth - GroundWidth * 2,
    thirdHeight + GroundHeight,
    "ground"
  );
...
function ItemsToCollect(parent) {
  ...
  SpeedBuff = parent.physics.add.group({
    key: "speedBuff",
    setXY: {
      x: WindowWidth - speedRockWidth,
      y: WindowHeight - speedRockHeight - 20,
    },
  });
...
function CollectStar(player, star) {
  ...
  if (Stars.countActive(true) <= 0) {
    var x =
      player.x < 400
        ? Phaser.Math.Between(400, CenterWidth)
        : Phaser.Math.Between(10, CenterWidth);
...
function MuteButton(parent) {
  ...
  soundButton.setDisplaySize(
    soundButton.displayWidth / 10,
    soundButton.displayHeight / 10
  );
  soundButton.setInteractive({ useHandCursor: true });

  var muteButton = parent.add.image(16, 0, "mute-icon");
  muteButton.setOrigin(0, 0);
  muteButton.setDisplaySize(
    muteButton.displayWidth / 10,
    muteButton.displayHeight / 10
  );
  muteButton.visible = false;
  muteButton.setInteractive({ useHandCursor: true });

  soundButton.on("pointerdown", () => {
    ToggleMute(soundButton, muteButton);
  });
...
function ScoreDisplay(parent) {
  ScoreText = parent.add.text(16, 16, "Score: 0", {
    fontSize: "32px",
    fill: "#fff",
  });
}
...
function KeybindingConfig(parent) {
  parent.mainAttack1 = parent.input.keyboard.addKey(
    Phaser.Input.Keyboard.KeyCodes.Q
  );
  parent.mainAttack2 = parent.input.keyboard.addKey(
    Phaser.Input.Keyboard.KeyCodes.W
  );
  parent.specialAttack1 = parent.input.keyboard.addKey(
    Phaser.Input.Keyboard.KeyCodes.E
  );
  parent.specialAttack2 = parent.input.keyboard.addKey(
    Phaser.Input.Keyboard.KeyCodes.R
  );
  parent.jumpAlt = parent.input.keyboard.addKey(
    Phaser.Input.Keyboard.KeyCodes.SPACE
  );
  parent.run = parent.input.keyboard.addKey(
