function WarriorController(parent) {
    ...
    var warriorPlayer = game.playerMap[CurrentClientId];
    var playerInfo = {};
    ...
    if (!IsGameOver && warriorPlayer != null && warriorPlayer != undefined) {
        ...
        var currentWarrior = 'warrior' + (game.DoesPlayerHasStrengthBuff ? '2' : '1');
        ...
        if (Cursors.left.isDown) {
            ...
            warriorPlayer.setVelocityX((game.DoesPlayerHasSpeedBuff ? -300 : -190) * (parent.run.isDown ? 1.65 : 1));
        ...
        //Player Not Moving
            ...
            if (parent.mainAttack1.isDown) {
                ...
                warriorPlayer.anims.play(currentWarrior + '-attack' + (game.DoesPlayerHasStrengthBuff ? 1 : 2) + '-' + (game.IsMainPlayerFacingLeft ? 'left' : 'right'), true);
                ...
                warriorPlayer.anims.play(currentWarrior + '-attack2-' + (game.IsMainPlayerFacingLeft ? 'left' : 'right'), true);
            ...
            else if (parent.specialAttack1.isDown && !game.IsWarriorSpecialAttack1Used) {
                game.IsPlayerIdle = false;
                playerInfo.IsPlayerIdle = false;

                warriorPlayer.anims.play(currentWarrior + '-attack' + ((game.DidWarriorAttackOnce && game.DoesPlayerHasStrengthBuff) ? 1 : 2) + '-' + (game.IsMainPlayerFacingLeft ? 'left' : 'right'), true);

                if (warriorPlayer.anims.currentFrame.index >= 14) {
                    playerInfo.UsingWarriorSpecialAttack1 = true;
                    game.WarriorSpecialAttack1 = CreateWarriorSpecialAttack1(parent, warriorPlayer);

                    if (game.IsMainPlayerFacingLeft) {
                        game.WarriorSpecialAttack1.enableBody(true, warriorPlayer.x - 32, warriorPlayer.y, true, true);
                        game.WarriorSpecialAttack1.setVelocityX(game.DoesPlayerHasStrengthBuff ? -350 : -250);
                        game.WarriorSpecialAttack1.setAccelerationX(game.DoesPlayerHasStrengthBuff ? -300 : -150);
                    }
                    else {
                        game.WarriorSpecialAttack1.enableBody(true, warriorPlayer.x + 32, warriorPlayer.y, true, true);
                        game.WarriorSpecialAttack1.setVelocityX(game.DoesPlayerHasStrengthBuff ? 350 : 250);
            ...
            else if (parent.specialAttack2.isDown && !game.IsWarriorSpecialAttack2Used) {
                game.IsPlayerIdle = false;
                playerInfo.IsPlayerIdle = false;
                ...
                if (warriorPlayer.anims.currentFrame.index >= 14) {
                    ...
                    game.WarriorSpecialAttack2 = CreateWarriorSpecialAttack2(parent, warriorPlayer);

                    if (game.IsMainPlayerFacingLeft) {
                        game.WarriorSpecialAttack2.enableBody(true, warriorPlayer.x - 32, warriorPlayer.y, true, true);
                        game.WarriorSpecialAttack2.setVelocityX(game.DoesPlayerHasStrengthBuff ? -350 : -250);
                        game.WarriorSpecialAttack2.setAccelerationX(game.DoesPlayerHasStrengthBuff ? -300 : -150);
                    }
                    else {
                        game.WarriorSpecialAttack2.enableBody(true, warriorPlayer.x + 32, warriorPlayer.y, true, true);
                        game.WarriorSpecialAttack2.setVelocityX(game.DoesPlayerHasStrengthBuff ? 350 : 250);
            ...
            //Idle Left
                ...
                warriorPlayer.anims.play(currentWarrior + '-idle' + (game.IsPlayerIdle ? '2' : '1') + '-left', true);
            }
            //Idle Right
            else if (!IsMainPlayerFacingLeft && !warriorPlayer.body.isMoving) {
                warriorPlayer.anims.play(currentWarrior + '-idle' + (game.IsPlayerIdle ? '2' : '1') + '-right', true);
            }
        }
        ...
        if ((Cursors.up.isDown || parent.jumpAlt.isDown) && warriorPlayer.body.touching.down) {
            game.IsPlayerIdle = false;
            warriorPlayer.setVelocityY((game.DoesPlayerHasStrengthBuff ? -670 : -550));
            warriorPlayer.anims.play(currentWarrior + '-jump-' + (game.IsMainPlayerFacingLeft ? 'left' : 'right'), true);
...
function UpdateWarriorAttacks() {
    if (game.WarriorSpecialAttack1 != null && game.WarriorSpecialAttack1 != undefined) {
        if (game.IsMainPlayerFacingLeft) {
    ...
    if (game.WarriorSpecialAttack2 != null && game.WarriorSpecialAttack2 != undefined) {
        if (game.IsMainPlayerFacingLeft) {
            game.WarriorSpecialAttack2.flipX = true;
            game.WarriorSpecialAttack2.angle -= game.DoesPlayerHasStrengthBuff ? 12 : 6;
        }
        else {
...
function DestroyWarriorAttacks() {
    ...
    if (game.WarriorSpecialAttack1 != null && game.WarriorSpecialAttack1 != undefined && game.WarriorSpecialAttack1.body != null && game.WarriorSpecialAttack1.body != undefined) {
        if (game.WarriorSpecialAttack1.body.onWall() && !game.WarriorSpecialAttack1.body.onFloor() && game.WarriorSpecialAttack1.body.enable) {
            game.IsWarriorSpecialAttack1Used = false;
            game.WarriorSpecialAttack1.destroy();
        }
    }

    if (game.WarriorSpecialAttack2 != null && game.WarriorSpecialAttack2 != undefined && game.WarriorSpecialAttack2.body != null && game.WarriorSpecialAttack2.body != undefined) {
        if (game.WarriorSpecialAttack2.body.onWall() && !game.WarriorSpecialAttack2.body.onFloor() && game.WarriorSpecialAttack2.body.enable) {
...
function UpdateClientWarrior(playerInfo) {
    if (CurrentClientId != -1) {
        ...
        if (player != null && player != undefined) {
            var currentWarrior = 'warrior' + (playerInfo.DoesPlayerHasStrengthBuff ? '2' : '1');
            ...
            if (playerInfo.IsWalkingLeft) {
                player.setVelocityX((playerInfo.DoesPlayerHasSpeedBuff ? -300 : -190) * (playerInfo.IsRunning ? 1.65 : 1));
            ...
            //Player Right
                ...
                if (playerInfo.IsMainAttack1Down) {
                    player.anims.play(currentWarrior + '-attack' + (playerInfo.DoesPlayerHasStrengthBuff ? 1 : 2) + '-' + (playerInfo.IsMainPlayerFacingLeft ? 'left' : 'right'), true);
                    ...
                    player.anims.play(currentWarrior + '-attack2-' + (playerInfo.IsMainPlayerFacingLeft ? 'left' : 'right'), true);
                ...
                //Cast Special Attack
                else if (playerInfo.IsSpecialAttack1Down && !playerInfo.IsWarriorSpecialAttack1Used) {

                    player.anims.play(currentWarrior + '-attack' + ((playerInfo.DidWarriorAttackOnce && playerInfo.DoesPlayerHasStrengthBuff) ? 1 : 2) + '-' + (playerInfo.IsMainPlayerFacingLeft ? 'left' : 'right'), true);

                    if (player.anims.currentFrame.index >= 14) {
                        playerInfo.UsingWarriorSpecialAttack1 = true;
                        ClientWarriorSpecialAttack1Sprite = CreateWarriorSpecialAttack1(parent, player);

                        if (game.IsMainPlayerFacingLeft) {
                            ClientWarriorSpecialAttack1Sprite.enableBody(true, player.x - 32, player.y, true, true);
                            ClientWarriorSpecialAttack1Sprite.setVelocityX(playerInfo.DoesPlayerHasStrengthBuff ? -350 : -250);
                            ClientWarriorSpecialAttack1Sprite.setAccelerationX(playerInfo.DoesPlayerHasStrengthBuff ? -300 : -150);
                        }
                        else {
                            ClientWarriorSpecialAttack1Sprite.enableBody(true, player.x + 32, player.y, true, true);
                            ClientWarriorSpecialAttack1Sprite.setVelocityX(playerInfo.DoesPlayerHasStrengthBuff ? 350 : 250);
                ...
                //Cast Special Attack
                else if (playerInfo.IsSpecialAttack2Down && !playerInfo.IsWarriorSpecialAttack2Used) {
                    player.anims.play(currentWarrior + '-attack' + ((playerInfo.DidWarriorAttackOnce && playerInfo.DoesPlayerHasStrengthBuff) ? 1 : 2) + '-' + (playerInfo.IsMainPlayerFacingLeft ? 'left' : 'right'), true);

                    if (player.anims.currentFrame.index >= 14) {
                        playerInfo.UsingWarriorSpecialAttack2 = true;
                        ClientWarriorSpecialAttack2Sprite = CreateWarriorSpecialAttack2(parent, player);

                        if (playerInfo.IsMainPlayerFacingLeft) {
                            ClientWarriorSpecialAttack2Sprite.enableBody(true, player.x - 32, player.y, true, true);
                            ClientWarriorSpecialAttack2Sprite.setVelocityX(playerInfo.DoesPlayerHasStrengthBuff ? -350 : -250);
                            ClientWarriorSpecialAttack2Sprite.setAccelerationX(playerInfo.DoesPlayerHasStrengthBuff ? -300 : -150);
                        }
                        else {
                            ClientWarriorSpecialAttack2Sprite.enableBody(true, player.x + 32, player.y, true, true);
                            ClientWarriorSpecialAttack2Sprite.setVelocityX(playerInfo.DoesPlayerHasStrengthBuff ? 350 : 250);
                ...
                //Idle Left
                    ...
                    player.anims.play(currentWarrior + '-idle' + (playerInfo.IsPlayerIdle ? '2' : '1') + '-left', true);
                }
                //Idle Right
                else if (!playerInfo.IsMainPlayerFacingLeft && !player.body.isMoving) {
                    player.anims.play(currentWarrior + '-idle' + (playerInfo.IsPlayerIdle ? '2' : '1') + '-right', true);
                }
            }
...
function DestroyClientWarriorAttacks() {
    if (ClientWarriorSpecialAttack2Sprite != null && ClientWarriorSpecialAttack2Sprite != undefined && ClientWarriorSpecialAttack2Sprite.body != null && ClientWarriorSpecialAttack2Sprite.body != undefined) {
        ClientWarriorSpecialAttack2Sprite.destroy();
    }

    if (ClientWarriorSpecialAttack1Sprite != null && ClientWarriorSpecialAttack1Sprite != undefined && ClientWarriorSpecialAttack1Sprite.body != null && ClientWarriorSpecialAttack1Sprite.body != undefined) {
